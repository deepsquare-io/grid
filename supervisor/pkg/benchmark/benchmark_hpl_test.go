...
func TestGenerateHPLPhase2JobDefinition(t *testing.T) {
	// Arrange
	tests := []struct {
		title       string
		p           uint64
...
curl -fsSL \
  ...
  "$LOG_FILE" \
  -H "X-Secret: %s" \
  "https://localhost:3000/benchmark/hpl/phase2?nodes=3&cpusPerNode=16&gpusPerNode=2&memPerNode=128460"
`, base64.StdEncoding.EncodeToString(secret.Get())),
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.title, func(t *testing.T) {
			// Act
			b, err := GeneratePhase2HPLBenchmark(
				tt.p,
				tt.q,
				tt.problemSize,
				tt.blockSize,
